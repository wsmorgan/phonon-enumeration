...
def perm(casei,colors,length,index,gen,stab,order,ast):
	...
	li = list(irn.invhash(casei,colors,length))
        ...
        if index == 0:
                #first apply all the symmetry operations one at a
                #time, a is a single operation
                for a in gen:
                        ...
                        # any color that was placed that isn't the
                        # color for the current level is set to zero
                        ...
                        #hash from radix_num_generator turns the array
                        #back into an integer hash array
			rtest = rng.hash(list(lnewb),colors)

                        #if the original hash is smaller than the new
                        #one we don't want to visit the new one again
                        #so we set its location in order to be -1
                        if casei[0] < rtest:
                                order[rtest] = -1

                        #if the original hash is larger than the new
                        #one then this configuration isn't unique so
                        #we set unique to be 1 and reset the other
                        #variables
			elif casei[0] > rtest:
				unique += 1
                                st = []
				...
				break
                        ...
                        #if the symmetry group didn't change this
                        #level of the hash then it is a stabilizer and
                        #needs to be saved
                        elif casei[0] == rtest:
                                st.append(a)
        ...
        #if there is only a single stabilizer then the configuration
        #is unique and the stabilizer needs to be used for any later
        #braches
        elif len(stab) == 1:
                st = stab
                ast = stab

        #if there are multiple stabilizers then we need to apply each
        #of them just like we needed to apply the entire group on the
        #first level of the tree
        elif len(stab) > 1:
                ast = []
                #first apply all the stabilizers one at a time, a is a
                #single operation
                for a in stab:
                        # get the effect of the group on the colors, ingore the arrows
                        # i is the permutation of the colors for this operation
                        ...
                        #if the the new and original configurations
                        #are the same then save the stabilizers
                        if lnewb == lib:
                                st.append(a)
                        if li == lnew:
                                ast.append(a)
                        ...
                        # if the configuration isn't unique break from
                        # the loop
                        if unique == 1:
                                ast = []
                                st = []
...
def brancher(concs,group,colors_w_arrows, dim, subset = []):
        ...
        # order is used to track which branchs of the first layer we
        # have already seen so we can skip over the ones we already
        # know aren't unique
        ast = []
        order = {}
        ...
        #stabilizers for the arrow configurations
                ...
                #if this array is unique then we may need to append it
                ...
                if unique == 0:
                        ...
                        if b0 == 1 or i == 0:
                                ...
                                if narrows > 0:
                                        ...
                                        #permutiation code by adding
                                        #arrows back into the array
                                        #where needed.
                                        for z in range(len(brancht)):
                                                brancht[z] = deepcopy(colors[brancht[z] -1])

                                        # add_arrows from the
                                        # phonon_brancher code returns
                ...
                # all that follows dictates how we navigate the
                # tree. This can be a little tricky but does work at
                # this time. Basically try not to mess with the last
                # few lines of this code if it can be helped
                                
                # if b0 is 0 then we need to move up to the next level
                # of the tree at this point
                if b0 == 0:
                        i += 1
                        # if we are now looking at the last level then
                        # set b0 to 1 so that any unique
                        # configurations found will be saved
                        if i == len(branch) - 2:
                                b0 = 1

                # root is a variable that tells us if we've filled
                # this level of the tree yet
                root = 0
                # if root = 0 and i>0 then we need to check to see if
                # we need to back track up the tree yet
                while root == 0 and i > 0:
                        # if we've used all the configurations for
                        # this level then we need to go back a level,
                        # if not then set root to 1 so that we keep
                        # moving forward through the tree
                        if branch[i] == C[i]-1:
                                # first reset this level's number so
                                # that it will increment properly the
                                # next time it's visited
                                branch[i] = 0
                                # next reduce i by 1 to go back a level
                                i -= 1
                                # if i = 0 we're back to the bottom of
                                # the tree. We need to reset the
                                # stabilizers, b0 and increment the
